...
function buttonLoadingAndCheck(event) {
    var min = parseInt(event.currentTarget.getAttribute('data-min-dur'), 10);
    var max = parseInt(event.currentTarget.getAttribute('data-max-dur'), 10);
    event.currentTarget.classList.add("buttonLoading_animate");
    setTimeout(function () {
        event.target.classList.remove("buttonLoading_animate");
        var check = event.target.querySelector("span span");
...
function listenersToButtonAnim() {
    var buttons = document.querySelectorAll('.buttonLoading');
    for (var i = 0; i < buttons.length; i++) {
...
function addToCardQ(event) {
    var min = parseInt(event.currentTarget.getAttribute('data-min-dur'), 10);
    var max = parseInt(event.currentTarget.getAttribute('data-max-dur'), 10);
    var id = parseInt(event.currentTarget.getAttribute('value'), 10);
    var quantity = parseInt(event.currentTarget.parentElement.querySelector(".counter__number").innerText, 10);
    var item = newCardItem(id, pizzas[id].name, pizzas[id].price, pizzas[id].picture, quantity, pizzas[id].price);
    var trs = document.querySelectorAll(".card__list tr");
    event.currentTarget.classList.add("buttonLoading_animate");
    setTimeout(function () {
        event.target.classList.remove("buttonLoading_animate");
...
function addToCard(event) {
    ...
    var id = parseInt(event.currentTarget.getAttribute('value'), 10);
    var item = newCardItem(id, pizzas[id].name, pizzas[id].price, pizzas[id].picture, 1, pizzas[id].price);
    var trs = document.querySelectorAll(".card__list tr");
    $('.card__counter .counter__number span').unbind('DOMSubtreeModified');
    insertAfter(trs[trs.length - 1], item);
    $('.card__counter .counter__number span').bind('DOMSubtreeModified', function (event) {
...
function removeItemsFromCard() {
    var trs = document.querySelectorAll(".card__list table tr");
...
function showPending(event) {
    ...
    var error = document.querySelector(".order__error");
    error.innerText = ans;
...
function counterUp(event) {
    var num = event.currentTarget.parentElement.parentElement.querySelector(".counter__number span");
...
function addEventListenersToCounters() {
    ...
    for (var i = 0; i < counters.length; i++) {
        counters[i].querySelector('.counter__up').addEventListener('click', function (event) {
            var num = event.currentTarget.parentElement.parentElement.querySelector(".counter__number span");
            num.innerText = parseInt(num.innerText, 10) + 1;
        });
        counters[i].querySelector('.counter__down').addEventListener('click', function (event) {
            var num = event.currentTarget.parentElement.parentElement.querySelector(".counter__number span");
...
function calculateTotal() {
    var subtotals = document.querySelectorAll(".card__subtotal span");
...
function calculateSubtotal(event) {
    var item = event.target.parentElement.parentElement.parentElement.parentElement.parentElement;
    var subtotal = item.querySelector('.card__subtotal span');
    var price = parseInt(item.querySelector('.card__amount').innerText, 10);
    subtotal.innerText = parseInt(event.target.innerText, 10) * price;
    calculateTotal();
}
function smoothScrollId(id) {
    smoothScroll(id, 500, function () {
    });
}
...
$(document).ready(function () {
    ...
    var num = localStorage.getItem('pen');
